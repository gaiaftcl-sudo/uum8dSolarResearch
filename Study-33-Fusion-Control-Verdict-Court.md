...
3. **Intact at scale — it is not.** A float's precision is relative to its magnitude, so it goes blind at a scale plasma quantities reach — 2²⁴ ≈ 16.8M, the order of a tokamak plasma current in amperes and past at once in any finer unit or accumulated count ([the magnitude proof below](#the-invariant-in-action--the-meaning-survives-any-magnitude)); past that the two states are the same number and the verdict is *undefined*.
...
- **float32 goes blind at $2^{24} = 16{,}777{,}216$** — the same order as a tokamak plasma current in amperes (ITER's 15 MA is 89% of the way there), and crossed at once in any finer unit or any accumulated charge count. Past it the two states are the *same float*: the safety verdict is not wrong, it is **undefined**. **MEASURED.** *(Corrected 2026-09-04: an earlier wording said the blind point sat "below a single plasma current in amperes"; $2^{24} \approx 16.8$ MA is just above ITER's 15 MA, so the honest statement is "the same order, crossed at once in a finer unit" — the point stands, the example was off by a factor near 1.1.)*
