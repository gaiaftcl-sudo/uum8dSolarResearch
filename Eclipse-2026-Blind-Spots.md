...
## Story 1 — Eugene, Oregon 2023: the eclipse the magnitude model threw away

The frozen magnitude law sealed `2023-10-14-eugene-or` **dead cat** — inside its narrow annular gate window (only ~18 minutes at ≥80% ring coverage) the depletion never reached the 300‰ coupling line, so the scalar test said *no eclipse signature*.

The shear row says otherwise: **window 131,566 ppm · flank 0 ppm · excess 131,566 ppm** — deepest bin at 16:27:30 UT, riding its own max-obscuration second by **+517 s**, squarely in the measured bottomside-lag window. A textbook-shaped, perfectly confined, correctly traveling eclipse signature — just a *weak* one. It clears the confinement threshold (50,189 ppm) by 2.6×.

**The blindness:** magnitude thresholds discard weak-but-perfectly-shaped signals. Any survey that filters events by "depletion ≥ X%" silently deletes every marginal-geometry eclipse response — annular edges, path edges, low-Sun crossings — from the record. The shape-read recovers them.

## Story 2 — May 11, 2024: the phantom eclipse every scalar pipeline records

On May 11 the Gannon negative phase depressed TEC **67–73% below quiet baselines at all seven benchmark stations** — deeper than any real eclipse in the corpus. A magnitude model doesn't just miss here; it *hallucinates*: our own frozen law false-positived **7/7**, "detecting" an eclipse at every station under a cloudless, unobscured Sun.

The shear rows: window 667,827–733,664 ppm — and **flanks 665,528–724,580 ppm**. The excess collapses to 2,204–42,836 ppm, under threshold everywhere. The depletion is real; the *shape* is a flood, not a shadow.

**The blindness:** a scalar can say *that* TEC dropped, never *why*. Every magnitude-threshold eclipse study running near solar maximum is one storm away from recording an event that never happened — and cannot know it from its own data.

## Story 3 — Mazatlán, May 11: the inversion no average survives

Same storm, same day — but Mazatlán's row reads **window 471,758 · flank 539,170 · excess −67,412 ppm**. The gate window was *less* depleted than its flanks: the storm's negative phase has latitude and local-time structure that happened to spare those exact minutes.

**The blindness:** magnitude models average that structure away. A negative excess is physically meaningful — it says "whatever did this was structured *against* the shadow's timetable" — and no scalar summary can even represent the sign.
...
**The blindness:** every magnitude-vs-baseline method is hostage to its baseline's calendar. Season, solar rotation, and secular solar-cycle trend all masquerade as depletion. The traveling-lag clause is calendar-proof because its clock is the umbra, not the archive.

## Story 5 — Carbondale, twice: one ground, one law, two suns

The only station under both the 2017 and 2024 totality paths. Same grid cells, same instrument network, one full solar cycle apart: coupling **488‰ (2017, SC24 declining) vs 342‰ (2024, SC25 maximum)** — a 30% modulation of the eclipse response by the solar-cycle background, on identical ground.

**The blindness:** a fixed magnitude threshold calibrated in one cycle silently mis-calibrates in the next — too strict at maximum, too loose at minimum. The corpus measures the modulation instead of hiding it, and the confinement discriminant is threshold-stable across both regimes because flanks and window scale together.
...
A magnitude is one number; blindness is what one number cannot hold. Shape — confinement to the forcing's own minutes, coherence with the forcing's own clock — is what the shear read adds, and each story above is a class of event that scalar methods will keep glossing over **forever**, not until better tuning: the weak-but-true signal (1), the strong-but-false one (2), the structured impostor (3), the drifting calendar (4), and the moving calibration (5). The [prediction extensions](Eclipse-2026-Prediction-Registry.md) carry these lessons forward as sealed, falsifiable claims for August 12.
