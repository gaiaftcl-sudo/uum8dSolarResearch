...
func referenceFigures(_ why: String) {
    ...
    emit("  Nasdaq PSX TotalView-ITCH 5.0, 2019-07-30 — THE SAME DAY, A DIFFERENT BOOK")
    emit("    orders terminated                     16,165,067")
    emit("    never executed                        15,952,637")
    emit("    per 1,000 of orders                          986")
    emit("    per 1,000 of displayed shares                992")
    emit("    termination split   DELETE 13,582,830 · REPLACE 2,405,437 · DRAINED 176,800")
    ...
    emit("  Nasdaq ITCH v2 and BX 5.0 are sixteen years and a protocol generation apart,")
    emit("  and the reading moves by 2.3 points.  BX and PSX are the SAME DAY on two books")
    emit("  run by the same operator, and it moves by 2.8 — MORE in one session than in")
    emit("  sixteen years, with every macro variable held fixed.  The two books differ in")
    emit("  matching engine and fee model, and they differ on the wire: BX carries 15")
    emit("  message types, PSX 14, and the one BX has is N, the Retail Price Improvement")
    emit("  indicator, 4,636,704 of them.  PSX runs no such programme.")
    emit("  SO THE BASE RATE IS VENUE-DEPENDENT.  A detector keyed on cancellation, on")
    emit("  order lifetime, or on displayed liquidity that never trades measures MARKET")
    emit("  MAKING, and it measures a different amount of it on each book.  This is a")
    emit("  DENOMINATOR, never a detector, and the denominator has to name its venue.")
    emit("")
    emit("  IEX DEEP 1.0, 2019-07-30 — THE PREDICATE CANNOT RUN.  ABSENT, NOT ZERO.")
    emit("  DEEP publishes aggregated PRICE LEVELS, not orders: its price level update is")
    emit("  30 bytes, all 30 claimed by type, flags, timestamp, symbol, size and price,")
    emit("  leaving 0 bytes in which an order reference could sit.  A predicate keyed on an")
    emit("  order being opened and then terminated has no key here and no denominator, and")
    emit("  a per-1,000 rate computed on this feed would answer a different question.")
    emit("  Decided from the bytes by feed-order-identity.swift, not from a document.")
    emit("")
    ...
    emit("    PSX 2019-07-30    flag 169,275  control 57,401   2,948 per 1,000")
    emit("                      = 1,061 per 100,000 of the 15,952,637 phantom orders")
    emit("    ITCH v2 2003      flag  1,643   control  1,033   1,590 per 1,000")
    emit("    The composite separates from its own control on PSX MORE THAN TWICE as")
    emit("    strongly as on BX, same day, same binary.  Direction clears on three")
    emit("    sessions; magnitude spans 1,407 to 2,948 and is still not called.")
