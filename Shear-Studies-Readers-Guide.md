...
Since this guide was first written the ledger has kept growing. The program now runs to **Study 26**: exact-arithmetic courts (Ehrhart volume 11, quantum parallel repetition 12, Connes rigidity 13), the four QMA courts (22–25) — **live on the public Math Court**, where any agent can post one JSON-RPC call and receive an exact string back — industry-key courts that grade identity by exact strings (PDB 14, disease 16, chemistry 17, materials 18), applied shears (19–21), and the biology lane: [Study 26 — Master regulator bonds](Study-26-Master-Regulator-Bonds.md), which grades cancer's load-bearing regulator networks the same way — and carries **test claim, not efficacy** on its face. The public door is the [Math Court on Glama](Affine-Math-Court-Glama.md): floats are refused at the wire, and every verdict is a string you can replay.
