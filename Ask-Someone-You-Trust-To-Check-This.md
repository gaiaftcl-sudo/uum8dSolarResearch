...
And then it says — in the authors' own words — that the effect of this on the atmosphere **is unknown**.
...
When you cannot measure a thing directly, you take every published result, you run each one forward to the scale that is actually being proposed, and you look at the spread. You do not pick a winner. You lay out the envelope — and the size of the envelope tells you how much you are gambling.
...
|---|---|---|---|
| what is happening now | 1× | 86 mDU | 0.03% |
| what is already **approved** (19,408 satellites) | 9× | 771 mDU | 0.26% |
| the **100,000**-satellite application | 46× | 3,973 mDU | 1.3% |
| the **million**-satellite application | 377× | 32,302 mDU | **10.8%** |

To read that last row you need one comparison. **At the very worst of the CFC crisis — the aerosol cans, the refrigerants, the thing that got a global treaty and thirty years of repair — the global average ozone column dropped by about 5%.**
...
- **The multiplication is almost certainly too simple.** Chemistry like this saturates — as the sky fills with particles, each additional one does less. The real curve bends. It could bend a lot.
...
That is the width of what we do not know. Not a small uncertainty around a comfortable answer — a spread that runs from "fine" to "twice the CFC crisis", on a question about the layer of air that stops ultraviolet light reaching your skin, your crops, and the plankton that makes half your oxygen.
...
Barker and colleagues, modelling all space activity in 2026, found ozone loss dominated by **chlorine from solid rocket propellant** — a launch emission. In their analysis satellite mega-constellations account for only about 9% of the damage, and specifically *because* they fly kerosene rockets, which emit no chlorine at all.
...
There is no year when something snaps, no threshold that gets crossed, no moment when an alarm goes off — and **that is precisely the problem**, not a reason for comfort.

Here is the actual shape of it:

- The amount goes **up** — from nothing to nearly half of everything entering the atmosphere, in six years.
...
It gets worse when you look at why. Under the law, a "re-entering vehicle" means something built to come back **in one piece**. A satellite designed to burn up is not one — so the environmental review that would normally apply simply never applies. The category doesn't fit, so nobody has to look.

When the American aviation regulator wrote its report to Congress about exactly this — satellites from large constellations burning up in the atmosphere — the words *ozone*, *stratosphere*, and *air quality* **do not appear anywhere in it.** Not once. The entire report is about whether a piece might land on somebody.

And right now, today, there are two live proposals: one to exempt space operations from environmental review altogether on the grounds that space is outside anyone's territory, and one to waive thirteen environmental laws for launch companies — **including the Clean Air Act.**
...
For context on that number: the entire natural stratospheric aerosol layer — everything, all of it, built up over all of history — weighs a few hundred thousand tonnes. A constellation at that scale would put a comparable quantity through the stratosphere and *keep it there*, continuously, forever.
...
We are not saying anyone will die. We are not saying the ozone layer is collapsing — we just told you it is recovering, and linked the paper. We are not saying the scientists who found "no effect" are corrupt or wrong. We do not know that they are wrong. **That is the point.**
...
That question does not need the science settled. It gets *stronger* the more the scientists disagree — because disagreement is the reason to measure, not the reason to wait.
