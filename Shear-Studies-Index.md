...
|---|-------|--------|---------------------------|
| 1 | [Eclipse 2026 — Overview](Eclipse-2026-Overview.md) | **DATA SEALED** — first look 2026-08-12; frozen integers + 82-row corpus. Claim on apex is not served yet. | The Moon's umbral cone crossed Iberia on 2026-08-12 on a verified-quiet day; the raw first look shows the total-electron-content bite riding the shadow's ground track at all four custody stations, with geomagnetic storms as the sealed adversary. The sealed verdict grades on the pinned archive (~2026-08-30). |
| 2 | [Study 02 — Launch ionospheric holes](Study-02-Launch-Ionospheric-Holes.md) · [Regulatory alarm](Study-02-Regulatory-Alarm.md) | **LIVE CLAIM** · **DATA** Starlink 10-49 **2026-08-25T09:33:38Z** | Rocket second-stage burns punch F-region holes on a manifest-published clock. This morning’s Cape jellyfish is the weekly clock; 900 km stays **cited / not measured** until Madrigal TEC posts. Hole is terrestrial de-bias vs Study 07 — not a 2D DM verdict. Edge: *not known*. Alarm package ready; no agency receipt id. |
| 3 | [Study 03 — Solar-flare SIDs](Study-03-Solar-Flare-SIDs.md) · [Predictions & validations](Study-03-Predictions-and-Validations.md) | **OPEN** — no SID corpus; detection threshold not sealed | Layer A: sunlit TEC step on GOES clock vs storms/radio bursts. Layer B: SHARP signed crossing-count helicity (exact integer, not float ∫A·B) vs large-flux quiet ARs + SWAN-SF baseline — Moffatt linking on the Sun; torsion≠linking spelled out. |
...
| 5 | [Study 05 — Forbush decreases](Study-05-Forbush-Decreases.md) | **OPEN** — no NMDB corpus ingested; thresholds not frozen | An L1 shock minute strobes the ~50-station neutron-monitor network; a rigidity-ordered, longitude-flat global step must part — at zero false positives on raw uncorrected counts, replayed from archived snapshots against held-out negative days — from the pressure lows and the traveling diurnal wave that mimic its size but not its shape. |
| 6 | [Study 06 — Explosion vs earthquake](Study-06-Explosion-vs-Earthquake.md) · [Corpus](Study-06-Explosion-vs-Earthquake-Corpus.md) · [Results](Study-06-Explosion-vs-Earthquake-Results.md) · [Registry](Study-06-Explosion-vs-Earthquake-Registry.md) | **LAW FROZEN** — IC.MDJ 6/6 explosion WIN; 3/3 adversary reject. Data is sealed. Claim on apex is not served yet. | Catalog clock + 53±2 s travel + SNR ≥ 20,000 ppt on raw integer counts; near-site/collapse miss the appointment. IU.INCN VOID this seal. |
| 7 | [Study 07 — Sgr A\* Milky Way raw visibilities](Study-07-SgrA-Milky-Way-Raw-Visibilities.md) · [Corpus](Study-07-SgrA-Milky-Way-Corpus.md) · [Results](Study-07-SgrA-Milky-Way-Results.md) · [Registry](Study-07-SgrA-Milky-Way-Registry.md) | **LAW FROZEN** — primary `csv/` 8/8 WIN; `csv_norm/` adversary 8/8 MISS. Data is sealed. Claim on apex is not served yet. | Earth-rotation `(u,v)` tracks on Sgr A\* grade public EHT Stokes-I CSV rows (closures + radial ladder + arc order). Frozen integers: short ≥ 1.5 Jy, long ≤ 0.4 Jy, ratio ≥ ×5, closure concentration ≥ 200 ppt beating scramble null. Next portal product is a registry slot, not an open study. |
| 8 | [Study 08 — Gaia BH1 astrometric shear](Study-08-Gaia-BH1-Astrometric-Shear.md) | **OPEN** — no µas ingest; thresholds not frozen | Raw Gaia / RV vectors for the luminous star (integer µas, m·s⁻¹) vs the published *N* M☉ dark-companion fit as adversary — pipeline 1 of the three cosmology black-hole paths; ingest = dedicated corpus only, never float-thrash into core OS or NATS. |
...
| 10 | [Study 10 — Fermi gamma shear / dark matter](Study-10-Fermi-Gamma-Shear-Dark-Matter.md) | **CHARTER WITH ARCHIVE** — named HEASARC weekly photon FITS + paper; no keV ingest. Not OPEN. Edge word: *not known*. | ~43.2 GeV stacked cluster line vs **Inner Galaxy null**. Continuous stack = adversary. This row does not say dark matter is disproved. |
...
| 12 | [Study 12 — Quantum parallel repetition](Study-12-Quantum-Parallel-Repetition-Shear.md) | **LIVE CLAIM** | vQbit + Jordan bonds vs float amplitude tensor — discrete repetition bounds. Replaces many-electron \(\psi\). [Fourier Phantom](Impact-Study-Fourier-Phantom.md). |
| 13 | [Study 13 — Connes rigidity](Study-13-Connes-Rigidity-Shear.md) | **LIVE CLAIM** | Group rigidity via integer linking invariants vs float spectral adversary. Replaces KS potential. [Fourier Phantom](Impact-Study-Fourier-Phantom.md). |
